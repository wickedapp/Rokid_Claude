...
/** 从工具名+输入提炼给用户看的一句人话 + 用于记忆键的命令串。 */
export function summarize(tool: string, input: Record<string, unknown>): { summary: string; command: string } {
  const cmd =
    (typeof input.command === 'string' && input.command) ||
    (typeof input.file_path === 'string' && input.file_path) ||
    (typeof input.path === 'string' && input.path) ||
    '';
  const verb = tool === 'Bash' ? '运行' : tool === 'Write' || tool === 'Edit' ? '写' : tool;
